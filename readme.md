...
2. Install Composer using Homebrew via `brew install composer`.
3. Install Valet+ with Composer via `composer global require techdivision/valet-plus`.
4. Run the `~/.composer/vendor/bin/valet fix` command. This will check for common issues preventing Valet+ from installing.
5. Run the `~/.composer/vendor/bin/valet install` command. Optionally add `--with-mariadb` to use MariaDB instead of MySQL This will configure and install Valet+ and DnsMasq, and register Valet's daemon to launch when your system starts.
